...
def generate_code128_barcode(text, height, width):
    ...
    # Сохраняем во временный объект PIL Image (в памяти)(code128.)
    pil_img = code128.render(writer_options={"module_height": 15.0, "quiet_zone": 1.0})

    barcode_np = np.float32(pil_img.convert('L'))
    

    barcode_np = add_contrast_border_to_barcode(barcode_np)
 
    # Конвертируем в numpy array (grayscale)(pil_img)
    barcode_np = np.array(barcode_np.convert('L'))
...
def embed_alpha_blend(image, barcode_img, alpha=0.3):
    ...
    mask = np.any(barcode_img != [255,255,255], axis=2)

    roi_blue = image[:, :, 0]
    bc_blue = barcode_img[:, :, 0]

    blended_blue = ((1 - alpha) * roi_blue + alpha * bc_blue).astype(np.uint8)

    roi_blue[mask] = blended_blue[mask]
...
def embed_lsb(image, barcode_img):
    ...
    gray_bc = cv2.cvtColor(barcode_img, cv2.COLOR_BGR2GRAY)
    
    _, bin_bc_inv = cv2.threshold(gray_bc, 127, 1, cv2.THRESH_BINARY_INV)

    blue_channel = image[:, :, 0]
    
    blue_channel &= 0b11111110          
    blue_channel |= bin_bc_inv.astype(np.uint8)   
...
def add_contrast_border_to_barcode(barcode_img):
    # Преобразуем в grayscale и бинаризуем
    gray_bc = cv2.cvtColor(barcode_img, cv2.COLOR_BGR2GRAY)
    _, binary_bc = cv2.threshold(gray_bc, 127, 255, cv2.THRESH_BINARY)

    # Находим контуры чёрных элементов(255 -binary_bc) (инвертируем для поиска)
    contours, _ = cv2.findContours(binary_bc, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    # Копия изображения для рисования рамок
    barcode_with_borders = barcode_img.copy()

    # Настройки цвета и толщины рамки
    border_color = (255, 255, 255)   # черная рамка
    border_thickness = 2

    # Рисуем контуры (рамки)
    cv2.drawContours(barcode_with_borders, contours, -1, border_color, border_thickness)

    return barcode_with_borders
