...
def draw_outer_borders(img, color=(0,0,0), thickness=2):
    h, w = img.shape[:2]
...
def draw_per_bar_borders(img, color=(0,0,0), thickness=1):
...
def add_borders(barcode_img, outer_color=(0,0,0), per_color=(0,0,0)):
    bc = barcode_img.copy()
    draw_per_bar_borders(bc, color=per_color, thickness=1)
    draw_outer_borders(bc, color=outer_color, thickness=2)
    return bc
...
def preview_barcode():
    ...
    # Для предпросмотра выделяем рамки красным
    bc_red = add_borders(bc, outer_color=(0,0,255), per_color=(0,0,255))
    photo = cv2_to_photoimage(bc_red)
...
def process_and_embed():
    ...
    bc_full = generate_code128_barcode(text, *img.shape[:2])
    if var_ab.get(): img = embed_alpha_blend(img, bc_full, float(entry_alpha.get()))
    if var_lsb.get(): img = embed_lsb(img, bc_full)
...
# Строка ввода текста и кнопка предпросмотра
...
# Метка для превью штрихкода
preview_label = tk.Label(root)
preview_label.pack(pady=5)

# Опции встраивания
frame_opts = tk.LabelFrame(root,text="Опции")
